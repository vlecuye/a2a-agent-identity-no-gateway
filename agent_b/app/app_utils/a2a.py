...
def _resolve_app_url(app_url: str | None) -> str:
    ...
    if app_url:
        url = app_url
    elif env_url := os.getenv("APP_URL"):
        url = env_url
    else:
        ...
        if agent_engine_id and project and location:
            url = (
                f"https://{location}-aiplatform.mtls.googleapis.com/reasoningEngines/v1"
    ...
    if "-aiplatform.googleapis.com" in url:
        url = url.replace("-aiplatform.googleapis.com", "-aiplatform.mtls.googleapis.com")
    return url
